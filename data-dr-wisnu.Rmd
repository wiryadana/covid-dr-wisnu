...
gejala_by_severity <- data %>%
  select(9:22,45) %>% 
  tbl_summary(by=Derajat_new, missing = "no") %>%
  add_p(test=list(all_continuous() ~ "t.test", all_categorical() ~ "fisher.test"))
...
outcome_by_severity <- data %>%
  select(41,44:46) %>%
  tbl_summary(by=Derajat_new, statistic = list(all_continuous() ~ "{mean} ({sd})")) %>%
...
interaction3 <- ggplot(data = data, aes(x=Umur, y=Derajat_new)) +
  ...
  scale_color_manual(values = c("#5089C6","#FF3D68")) + # outline warna merah/oranye artinya HT
  scale_fill_manual(values = c("#FFFFFF", "#FFAA4C")) + # isi warna merah artinya DM
  scale_shape_manual(values=c(21,22,24)) +
  theme_minimal() +
...
los <- ggplot(data=data, aes(x=Durasi.Rawat.Inap, y=Derajat_new)) +
  ...
  scale_color_manual(values = c("#FF3D68", "#FFFFFF")) + # outline warna merah/oranye artinya rujuk
