...
biv_p%>% 
  as_flex_table() %>%
  save_as_docx(biv_p, path = "biv.p.table.docx", pr_section = sect_properties)


```

```{r eval=T}
drive_upload(media = "biv.p.table.docx", path = as_id("https://drive.google.com/drive/u/2/folders/1LcqP38WQYIIxNeWaoDAb85MZjCTYgnXh"))
```
...
anova_p <- anova_int[[1]][[1,"Pr(>F)"]]
#anova_text <- paste0("Anova", " ", "p=", anova_int[[1]][[1,"Pr(>F)"]]," ", "(p=",  ifelse(anova_p <= 0.001, print("<0,001"), NA)[1], ")") 
anova_text <- paste0("Anova", " ", "(p=",  ifelse(anova_p <= 0.001, print("<0,001"), NA)[1], ")") 
grob_anova <- grobTree(textGrob(anova_text, x=0.03,  y=0.95, hjust=0, gp=gpar(fontsize=14, fontface="italic")))
```
...
anova_p_sat <-anova_sat[[1]][[1,"Pr(>F)"]]
#anova_sat_text <- paste0("Anova", " ", "p=", anova_sat[[1]][[1,"Pr(>F)"]]," ", "(p=",  ifelse(anova_p_sat <= 0.001, print("<0,001"), NA)[1], ")") 
anova_sat_text <- paste0("Anova", " ", "(p=",  ifelse(anova_p_sat <= 0.001, print("<0,001"), NA)[1], ")") 
grob_anova_sat <- grobTree(textGrob(anova_sat_text, x=0.10,  y=0.95, hjust=0, gp=gpar(fontsize=14, fontface="italic")))
```
...
anova_p_los <-anova_los[[1]][[1,"Pr(>F)"]]
#anova_los_text <- paste0("Anova", " ", "p=", anova_los[[1]][[1,"Pr(>F)"]]," ", "(p=",  ifelse(anova_p_los <= 0.001, print("<0,001"), NA)[1], ")") 
anova_los_text <- paste0("Anova", " ", "(p=",  ifelse(anova_p_los <= 0.001, print("<0,001"), NA)[1], ")") 
grob_anova_los <- grobTree(textGrob(anova_los_text, x=0.60,  y=0.95, hjust=0, gp=gpar(fontsize=14, fontface="italic")))
...
```{r}
med.los_binary <- data %>%
  select(Durasi.Rawat.Inap, derajat_klinis) %>%
  tbl_summary(by=derajat_klinis,
  ...
  add_overall() %>%
  add_n() %>%
  modify_caption("<b>Table X.</b> Median Rawat Inap berdasarkan derajat klinis COVID-19") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Derajat Klinis COVID-19**") %>%
  modify_header(label ~ "**Variabel**") %>%
  bold_labels()
med.los_binary
...
```{r}
med.los_binary <- data %>%
  select(Durasi.Rawat.Inap, Derajat_new) %>%
  tbl_summary(by=Derajat_new,
              statistic = list(all_continuous() ~ "{median} ({p25}, {p75})", all_categorical() ~ "{n} ({p}%)"),
              percent = "row",
              missing = "no") %>%
...
## Bivariate & Multivariate {.tabset}

### Finding Cutoff Numeric Variable
#### Usia
```{r}
cut_umur <- cutpointr(data=data, 
          x=Umur, 
          class=derajat_klinis, 
          method = maximize_metric, 
          metric = sum_sens_spec)
s_cut_umur <- summary(cut_umur)
p_cut_umur <- plot(cut_umur)
s_cut_umur
p_cut_umur
```
Optimal Cut Point untuk **Umur** adalah **`r cut_umur$optimal_cutpoint`** tahun.

```{r}
data$usia_cut <- ifelse(data$Umur < 50, "Kurang dari 50", "Lebih dari/sama 50")
data$usia_cut <- factor(data$usia_cut, levels = c("Lebih dari/sama 50", "Kurang dari 50"))
```

#### Saturasi Oksigen 

```{r}
cut_sat <- cutpointr(data=data, 
          x=Saturasi, 
          class=derajat_klinis, 
          method = maximize_metric, 
          metric = sum_sens_spec)
s_cut_sat <- summary(cut_sat)
p_cut_sat <- plot(cut_sat)
s_cut_sat
p_cut_sat
```

Optimal Cut Point untuk **Saturasi** adalah **`r cut_sat$optimal_cutpoint`**%.

```{r}
data$saturasi_cut <- ifelse(data$Saturasi < 94, "Kurang dari 94%", "Lebih dari/sama 94%")
data$saturasi_cut <- factor(data$saturasi_cut, levels = c("Kurang dari 94%", "Lebih dari/sama 94%"))
```

### Bivariate odd_ratio

#### Recode Variabel Menjadi Binary
```{r}
### Xray
data$xray_binary <- as.character(data$Xray)
data$xray_binary <- ifelse(data$xray_binary == "normal", "Normal", "Abnormal")
data$xray_binary <- factor(data$xray_binary, levels = c("Abnormal", "Normal"))

### Gejala utama sesak
data$gej.utama.sesak <- as.character(data$Gejala.Utama)
data$gej.utama.sesak <- ifelse(data$gej.utama.sesak == "sesak", "Sesak Napas", "Bukan Sesak")
data$gej.utama.sesak <- factor(data$gej.utama.sesak, levels = c("Sesak Napas", "Bukan Sesak"))
```

#### Bivariate Analysis
```{r}
# data %>%
#   select(Jenis.Kelamin, usia_cut, gej.utama.sesak, saturasi_cut, xray_binary, peny.komorbid, derajat_klinis) %>%
#   tbl_summary(by=derajat_klinis,
#               statistic = list(all_continuous() ~ "{mean} ({sd})", all_categorical() ~ "{n} ({p}%)"),
#               percent = "row",
#               missing = "no") %>%
...
#   add_overall() %>%
#   add_n() %>%
#   modify_caption("<b>Table Y.</b> Data Jenis Kelamin, Sesak, Saturasi, Xray, Komorbid berdasarkan derajat klinis COVID-19") %>%
#   modify_spanning_header(c("stat_1", "stat_2") ~ "**Derajat Klinis COVID-19**") %>%
#   modify_header(label ~ "**Variabel**") %>%
#   bold_labels()
```
...
```{r}
data_biv <- data %>%
  select(4, peny.komorbid, 24:25, `komorbid CKD`, 48:49,51:52)

odd_r <- function(data, variable, by, ...) {
  #variable = deparse(substitute(variable))
  #by = deparse(substitute(by))
  o <-oddsratio(x=data[[variable]], y=as.factor(data[[by]]))
  odd <- style_sigfig(o$measure[2])
  ci <- style_sigfig(o$measure[c(4,6)]) %>% paste(collapse = ", ")
  str_glue("{odd} ({ci})")
}
...
biv_table <- data_biv %>%
  ...
  tbl_summary(by=derajat_klinis,
              ...
              missing = "no") %>%
  add_p(test= list(all_categorical() ~ "fisher.test")) %>%
  add_stat(fns = everything() ~ odd_r, fmt_fun = NULL, location = c(everything() ~ "label")) %>%
  modify_header(list(add_stat_1 ~ "**OR (95% CI)**", all_stat_cols() ~ "**{level}**")) %>%
  add_overall() %>%
  add_n() %>%
  modify_caption("<b>Table Y.</b> Rangkuman data demografi, klinis dan luaran berdasarkan derajat klinis COVID-19") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Derajat Klinis COVID-19**") %>%
  modify_header(label ~ "**Variabel**") %>%
  bold_labels() 
...
biv_table%>% 
  as_flex_table() %>%
  save_as_docx(biv_table, path = "biv.odd.table.docx", pr_section = sect_properties)

```

```{r eval=T}
drive_upload(media = "biv.odd.table.docx", path = as_id("https://drive.google.com/drive/u/2/folders/1LcqP38WQYIIxNeWaoDAb85MZjCTYgnXh"))
```

Jenis kelamin tidak berhubungan dengan derajat klinis.
Saturasi oksigen memberikan separasi 100 dari derajat klinis. Antara perfect predictor atau karena data yang kurang bervariasi. Jenis kelamin dan Saturasi oksigen tidak dimasukan ke Model Regresi Logistik.

### Binary Logistic Regression
```{r}
### Refactor
data$usia_cut <- factor(data$usia_cut, levels = c("Kurang dari 50", "Lebih dari/sama 50"))
data$xray_binary <- factor(data$xray_binary, levels = c("Normal", "Abnormal"))
data$gej.utama.sesak <- factor(data$gej.utama.sesak, levels = c("Bukan Sesak", "Sesak Napas"))
data$peny.komorbid <- factor(data$peny.komorbid, levels = c("tidak", "ada"))
data$`komorbid CKD` <- factor(data$`komorbid CKD`, levels = c("tidak", "ya"))
data$Komorbid_DM <- factor(data$Komorbid_DM, levels = c("tidak", "ya"))
data$derajat_klinis <- factor(data$derajat_klinis, levels = c("sedang", "berat/kritis"))



logres <- glm(derajat_klinis ~ usia_cut + gej.utama.sesak + xray_binary + Komorbid_HT + Komorbid_DM + `komorbid CKD`,
              data = data,
              family = binomial)
summary(logres)

multi <- tbl_regression(logres, 
               exponentiate = TRUE) %>%
  add_global_p() %>%
  bold_p(t = 0.05) %>%
  bold_labels() %>%
  italicize_levels() %>%
  modify_caption("<b>Table Z.</b> <i>Adjusted Odd Ratio</i> dari beberapa prediktor derajat klinis COVID-19") %>%
  modify_header(label ~ "**Variabel**") 
...
multi%>% 
  as_flex_table() %>%
  save_as_docx(multi, path = "multivariate.docx", pr_section = sect_properties)


```
```{r eval=T}
drive_upload(media = "multivariate.docx", path = as_id("https://drive.google.com/drive/u/2/folders/1LcqP38WQYIIxNeWaoDAb85MZjCTYgnXh"))
```
...
```{r}
### Refactor
data$Komorbid_HT <- factor(data$Komorbid_HT, levels = c("tidak", "ya"))
data$Komorbid_DM <- factor(data$Komorbid_DM, levels = c("tidak", "ya"))
#data$derajat_klinis <- factor(data$derajat_klinis, levels = c("sedang", "berat/kritis"))



logres2 <- glm(derajat_klinis ~ Komorbid_HT + Komorbid_DM,
              data = data,
              family = binomial)
summary(logres2)

multi2 <- tbl_regression(logres2, 
               exponentiate = TRUE) %>%
  add_global_p() %>%
  bold_p(t = 0.05) %>%
  bold_labels() %>%
...
* Lucy D'Agostino McGowan and Jennifer Bryan (2020). googledrive: An Interface to Google Drive. R package version 1.0.1. https://CRAN.R-project.org/package=googledrive
...
* Thiele C, Hirschfeld G (2021). “cutpointr: Improved Estimation and Validation of Optimal Cutpoints in R.” _Journal of Statistical Software_, *98*(11), 1-27. doi: 10.18637/jss.v098.i11 (URL:https://doi.org/10.18637/jss.v098.i11).

* Alboukadel Kassambara (2020). ggpubr: 'ggplot2' Based Publication Ready Plots. R package version 0.4.0. https://CRAN.R-project.org/package=ggpubr

* Tomas J. Aragon (2020). epitools: Epidemiology Tools. R package version 0.5-10.1. https://CRAN.R-project.org/package=epitools

* David Gohel (2021). officer: Manipulation of Microsoft Word and PowerPoint Documents. R package version 0.3.19. https://CRAN.R-project.org/package=officer

## Session Info
```{r, echo=FALSE}
package <- c("base", "googlesheets4", "tidyverse", "ggplot2", "gt", "gtsummary", "ggVennDiagram", "ggdist", "ggpubr", "patchwork", "DT", "cutpointr")
